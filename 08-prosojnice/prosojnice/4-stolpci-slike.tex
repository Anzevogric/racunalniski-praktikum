...
\begin{frame}
	...
	\begin{columns}
		\column{0.55\textwidth}
		  ...
		  \begin{itemize}
			 \item \onslide<1-> {Dani sta premica $p$ in točka $T$.}
			 \item \onslide<2-> {Nariši lok $k$ s središčem v $T$.}
			 \item \onslide<3-> {Premico $p$ seče v točkah $A$ in $B$.}
			 \item \onslide<4-> {Nariši lok $m$ s središčem v $A$.}
			 \item \onslide<5-> {Nariši lok $n$ s središčem v $B$ in z enakim polmerom.}
			 \item \onslide<6-> {Loka se sečeta v točki $C$.}
			 \item \onslide<7-> {Premica skozi točki $T$ in $C$ je pravokotna na $p$.}
		  \end{itemize}
		\column{.45\textwidth}
		\centering
		\begin{tikzpicture}
		% Sliko mo naredili tako, da so točke A, B, T in C vse enako oddaljene
				 % od presečišča premic; kot ATC je 45°.
				 % Vsi krožni loki imajo radij 2.
				 \tikzmath{
				 	% Razdalja od točke T do premice p je tako 2*sin(45°).
